...
const monthStatsByPlan = computed((): ((planId: number) => { totalCheckins: number; activeDays: number; percentage: number }) => {
  return (planId: number) => {
    let totalCheckins = 0;
    let activeDays = 0;
    // Calculate days passed in this month up to today or end of month
    // But requirement says "本月天数" (Days in this month? Or days passed?)
    // Usually it means total checkins vs total expected days.
    // Let's count days in month that are in plan range.
  ...
  <view class="container">
    ...
    <view class="plan-list">
      <view v-for="plan in plansStore.items" :key="plan.id" class="plan-card" @click="handlePlanClick(plan.id)">
        ...
        <!-- Card Body: Left Calendar, Right Stats -->
