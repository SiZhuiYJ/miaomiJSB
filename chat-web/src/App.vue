...
const baseURL = ref(localStorage.getItem('chat.baseURL') || 'http://localhost:5167');
const token = ref(localStorage.getItem('chat.token') || '');
const meUserId = ref(Number(localStorage.getItem('chat.meUserId') || 0));
...
const api = computed(() => createChatApi(baseURL.value, token.value.trim()));
const selectedConversationId = computed(() => currentConversation.value?.id || 0);

watch([baseURL, token, meUserId], () => {
  localStorage.setItem('chat.baseURL', baseURL.value);
  localStorage.setItem('chat.token', token.value);
  localStorage.setItem('chat.meUserId', String(meUserId.value || 0));
});
...
async function loadConversations() {
  ...
  errorMessage.value = '';
  try {
    conversations.value = await api.value.getConversations();
  } catch (error: any) {
...
async function selectConversation(item: ConversationSummary) {
  ...
  errorMessage.value = '';
  try {
    currentConversation.value = await api.value.getConversation(item.id);
    messages.value = await api.value.getMessages(item.id);
    await markRead();
  } catch (error: any) {
    errorMessage.value = error?.response?.data?.message || error?.message || '加载会话失败';
  } finally {
...
async function createConversation() {
  ...
  errorMessage.value = '';
  try {
    const detail = await api.value.createConversation({
...
async function sendTextMessage() {
  ...
  errorMessage.value = '';
  try {
    await api.value.sendMessage(selectedConversationId.value, {
      messageType: 'text',
      content: composeText.value.trim(),
    });
    composeText.value = '';
    messages.value = await api.value.getMessages(selectedConversationId.value);
    await loadConversations();
  } catch (error: any) {
...
async function loadMore() {
  ...
  const older = await api.value.getMessages(selectedConversationId.value, firstMessage.id, 20);
  if (older.length > 0) {
...
async function markRead() {
  ...
  await api.value.markRead(selectedConversationId.value, lastId);
  await loadConversations();
}
    ...
    <div class="layout">
      ...
      <main class="main-panel">
        <div v-if="currentConversation" class="conversation-detail">
          ...
          <p class="members">成员：{{ currentConversation.members.map((m) => `${m.nickName || m.userId}(${m.memberRole})`).join('、') }}</p>
