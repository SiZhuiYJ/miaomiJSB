...
<template>
  <aside class="side panel">
    <div class="side__header">
      ...
      <input ref="queueFileInputRef" accept="image/*" :multiple="queueUploadAction?.mode === 'insert'" type="file"
        hidden @change="handleQueueFileChange">
    </div>
    <div class="side__scroll">
      <div class="thumbs" :class="{ 'is-sorting': imageQueueDragState.active }" aria-label="图片队列">
        ...
        }" :data-image-queue-index="index" role="button" tabindex="0" :aria-label="`拖拽排序 ${image.name}`"
          @contextmenu.prevent @pointerdown="startImageQueueDrag($event, index)">
    ...
    <div v-if="selectedImage && selectedCell" class="side__footer">
      <div class="meta">
        <strong>当前图片</strong>
        <span>{{ selectedImage.naturalWidth }} × {{ selectedImage.naturalHeight }}</span>
        <span class="meta__name">{{ selectedImage.name }}</span>
