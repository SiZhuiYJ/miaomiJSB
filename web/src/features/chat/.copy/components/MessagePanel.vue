    ...
    <div v-if="currentConversation" class="conversation-detail">
      <el-scrollbar ref="scrollbarRef" wrap-style="" view-class="">
        <div class="message-list" v-if="props.meUserId">
          <message-item v-for="msg in props.messages" :key="msg.id" @vue:mounted="onMessageVisible(msg.id)"
            :message="msg" :src="getMemberAvatarBySender(msg.senderUserId)" :read-text="getReadCountText(msg.id)"
            :readColor="getReadIconColor(msg.id)" :meUserId="props.meUserId" :createdAt="formatChatTime(msg.createdAt)"
            :is-mine="(props.meUserId != undefined && msg.senderUserId === props.meUserId)" />
        </div>
      </el-scrollbar>
