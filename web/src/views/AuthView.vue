...
async function validateuserAccount(): Promise<void> {
  ...
  try {
    ...
    if (error.response?.status === 409) {
      ...
      userAccountError.value = ''; // Ignore other errors or handle them if needed
    }
  }
}
  ...
  try {
    if (mode.value === 'login') {
      if (loginMethod.value === 'email') {
        const response = await http.post('/mm/Auth/login', {
          email: email.value,
          password: password.value,
        });
        auth.setSession(response.data);
      } else {
        const response = await http.post('/mm/Auth/login-account', {
      ...
      const response = await http.post('/mm/Auth/register', {
        email: email.value,
        password: password.value,
...
<template>
  <div class="auth-container">
    <div class="auth-card">
      ...
      <div class="tabs">
        <button type="button" :class="['tab', mode === 'login' ? 'active' : '']" @click="switchMode('login')">
          登录
        </button>
        <button type="button" :class="['tab', mode === 'register' ? 'active' : '']" @click="switchMode('register')">
          注册
        </button>
      </div>
      ...
      <form class="form" @submit.prevent="handleSubmit">
        <div class="login-methods" v-if="mode === 'login'">
          ...
          <label class="radio-label">
            <input type="radio" value="account" v-model="loginMethod" /> 账号登录
          </label>
        </div>

        <label class="field" v-if="mode === 'login' && loginMethod === 'email' || mode === 'register'">
          <span>邮箱</span>
          <input v-model="email" type="email"
            :required="mode === 'login' && loginMethod === 'email' || mode === 'register'" placeholder="请输入邮箱" />
        </label>

        <div v-if="mode === 'register'" class="field code-field">
          <span>邮箱验证码</span>
          <div class="code-row">
            <input v-model="code" type="text" maxlength="6" inputmode="numeric" placeholder="请输入收到的验证码" />
            <button type="button" class="code-button" :disabled="sendingCode || countdown > 0" @click="handleSendCode">
        ...
        <label class="field" v-if="mode === 'login' && loginMethod === 'account' || mode === 'register'">
          <span>账号</span>
          <div class="input-group">
            <input v-model="userAccount" type="text" :required="mode === 'login' && loginMethod === 'account'"
              placeholder="请输入账号" @blur="validateuserAccount" @input="userAccountError = ''"
              :class="{ 'input-error': userAccountError }" />
            <button v-if="mode === 'register'" type="button" class="btn-sm" @click="generateRandomuserAccount">随机</button>
          </div>
          <span v-if="userAccountError" class="error-msg">{{ userAccountError }}</span>
        </label>
        ...
        <label v-if="mode === 'register'" class="field">
          <span>确认密码</span>
          <input v-model="confirmPassword" type="password" required placeholder="请确认密码" />
        </label>
