    ...
    <div class="board-wrap" :style="boardWrapStyle">
      <div ref="boardRef" class="puzzle-board" :style="{ background: gapStyle === 'line' ? lineColor : '#fff' }">
        <article
          ...
          :key="cellInfo.index"
          class="puzzle-cell"
          :class="{ selected: selectedIndex === cellInfo.index }"
          :style="getCellStyle(cellInfo.row, cellInfo.col)"
          @click="selectCell(cellInfo.index)"
        >
          <template v-if="cellInfo.cell && cellInfo.image">
            <div
              class="cell-image-mask"
              @pointerdown="startImageDrag($event, cellInfo.index)"
              @wheel.prevent="zoomCellFromWheel($event, cellInfo.index)"
            >
              <img
                class="cell-image"
                :src="cellInfo.image.src"
                :alt="cellInfo.image.name"
                :style="getImageStyle(cellInfo.row, cellInfo.col, cellInfo.cell, cellInfo.image)"
                draggable="false"
              >
            </div>
          </template>
        ...
        <template v-if="images.length">
          <button
            v-for="segment in horizontalLineSegments"
            :key="segment.id"
            class="seam-line seam-line--horizontal"
          ...
          />
          <button
            v-for="segment in verticalLineSegments"
            :key="segment.id"
            class="seam-line seam-line--vertical"
            :style="getLineStyle(segment)"
            type="button"
            aria-label="拖动垂直分割线"
            @pointerdown="startSegmentDrag($event, segment)"
