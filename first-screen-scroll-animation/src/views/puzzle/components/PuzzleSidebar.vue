  ...
  <aside class="side panel">
    ...
    <div class="thumbs" :class="{ 'is-sorting': imageQueueDragState.active }" aria-label="图片队列">
      <button
        ...
        :data-image-queue-index="index"
        type="button"
        :aria-label="`拖拽排序 ${image.name}`"
        @contextmenu.prevent
    ...
    <div v-if="selectedImage && selectedCell" class="meta">
      ...
      <div class="zoom-panel">
        ...
        <div class="zoom-controls">
          ...
          <input
            :value="selectedCell.zoom"
            :style="getRangeStyle(selectedCell.zoom, MIN_IMAGE_ZOOM, MAX_IMAGE_ZOOM)"
            :min="MIN_IMAGE_ZOOM"
            :max="MAX_IMAGE_ZOOM"
            step="0.01"
            type="range"
            aria-label="调整当前区域图片缩放"
